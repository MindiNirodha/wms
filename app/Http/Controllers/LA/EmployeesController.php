...
class EmployeesController extends Controller
{
	...
	public function dtajax()
	{
		if(Auth::user()->hasRole('SUPER_ADMIN')){
			...
			$values      = DB::table('employees')->select($this->listing_cols)->whereNull('deleted_at')->where('dept','=',$instId->dept);
		}
		...
		for($i=0; $i < count($data->data); $i++) {
			for ($j=0; $j < count($this->listing_cols); $j++) { 
				...
				if($col == $this->view_col) {
					$data->data[$i][$j] = '<a href="'.url(config('laraadmin.adminRoute') . '/employees/'.$id).'">'.$data->data[$i][$j].'</a>';
				}
				// else if($col == "author") {
				//    $data->data[$i][$j];
				// }
			...
			if($this->show_action) {
				$output = '';
				if(Module::hasAccess("Employees", "edit")) {
					$output .= '<a href="'.url(config('laraadmin.adminRoute') . '/employees/'.$id.'/edit').'" class="btn btn-warning btn-xs" style="display:inline;padding:2px 5px 3px 5px;"><i class="fa fa-edit"></i></a>';
				}
				
				if(Module::hasAccess("Employees", "delete")) {
					$output .= Form::open(['route' => [config('laraadmin.adminRoute') . '.employees.destroy', $id], 'method' => 'delete', 'style'=>'display:inline']);
	...
	public function change_password($id, Request $request) {
		...
		return redirect(config('laraadmin.adminRoute') . '/employees/'.Crypt::encrypt($id).'#tab-account-settings');
	}
}
