...
<div id="page-content" class="profile2">
	<div class="bg-success clearfix">
		...
		<div class="col-md-1 actions">
			@la_access("Employees", "edit")
				<a href="{{ url(config('laraadmin.adminRoute') . '/employees/'.Crypt::encrypt($employee->id).'/edit') }}" class="btn btn-xs btn-edit btn-default"><i class="fa fa-pencil"></i></a><br>
			@endla_access
			
			@la_access("Employees", "delete")
				{{ Form::open(['route' => [config('laraadmin.adminRoute') . '.employees.destroy', Crypt::encrypt($employee->id)], 'method' => 'delete', 'style'=>'display:inline']) }}
	...
	<ul data-toggle="ajax-tab" class="nav nav-tabs profile" role="tablist">
		...
		@if($employee->id == $getContxtId->context_id || Entrust::hasRole("SUPER_ADMIN"))
	...
	<div class="tab-content">
		...
		<div role="tabpanel" class="tab-pane fade" id="tab-account-settings">
			<div class="tab-content">
				<form action="{{ url(config('laraadmin.adminRoute') . '/change_password/'.Crypt::encrypt($employee->id)) }}" id="password-reset-form" class="general-form dashed-row white" method="post" accept-charset="utf-8">
					{{ csrf_field() }}
					<div class="panel">
						<div class="panel-default panel-heading">
